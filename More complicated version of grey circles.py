...
def draw_ring_of_circles(num_circles, outer_circle_radius, pattern_radius):
    for counter in range(num_circles):
        ...
        turtle.left(180)
    #moving turtle away to the right
    turtle.seth(0)
    turtle.fd((pattern_radius+outer_circle_radius*2)*1.5)
    pass
...
def main():
    ...
    draw_inside_circle()
    turtle.seth(90) # face north
    turtle.fd(INSIDE_CIRCLE_RADIUS) #moving turtle to the centre of the second inner circle
    draw_ring_of_circles(9, 7, INSIDE_CIRCLE_RADIUS+8)
